...
class TestManifestDiscovery(object):
    ...
    def test_discover_transforms_ignore_test_dirs(
        self, load_manifest_at_path, transforms_fixtures_path, simple_transform_manifest_yml, tmpdir
    ):
...
class TestAppManifest(object):
    ...
    def test_run_app_one_job_multiple_steps(
        self, execute_job_steps, app_manifest_single_multiple_step_job, transforms_fixtures_path
    ):
  ...
  my-job:
    ...
    - transform: morgue-splitter
      ...
      output: /tmp/data/splits
        ...
        assert (
            str(exc.value)
            == "Invalid placeholder: $downloader.unknown; valid option names include: ['base_url', 'name', 'output', 'throttle', 'transform']"
        )
...
jobs:
  job1:
    ...
    @mock.patch("metl.core.runner.execute_job_steps")
    def test_resolve_data_dir(self, execute_job_steps, transforms_fixtures_path, tmpdir):
        ...
        actual_steps = execute_job_steps.call_args_list[0][1].get("steps") or execute_job_steps.call_args_list[0][0][1]
        assert all(
            step["output"] == actual_steps[0]["output"] for step in actual_steps
        ), "Every tmp value should be the same value"
  ...
  job1:
    ...
    - name: splitter
      ...
      output: /data/output
        ...
        assert (
            str(exc_info.value)
            == "No property named \"output\" defined in previous step. Possible values are: ['name', 'transform', 'base_url']"
        )
